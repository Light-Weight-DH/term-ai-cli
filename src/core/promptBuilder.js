...
export function buildUserPrompt({ requirementHistory, sessionSnapshot, includeSession, routeReason }) {
  const reqText = requirementHistory
    .map((r, i) => (i === 0 ? `요구사항: ${r}` : `추가정보${i}: ${r}`))
    .join("\n");
